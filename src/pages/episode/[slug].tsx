...
export default function Episode({ episode }: EpisodeProps) {
  ...
  const episodeMine = data.episodes.find(episodi => episodi.id === slug)
  console.log(episodeMine)
  console.log('episodeMine')

  // const episodeFake = {
  //   id: episodeMine.id,
  //   title: episodeMine.title,
  //   members: episodeMine.members,
  //   publishedAt: format(parseISO(episodeMine.published_at), 'd MMM yy', { locale: ptBR }),
  //   thumbnail: episodeMine.thumbnail,
  //   duration: Number(episodeMine.file.duration),
  //   durationAsString: convertDurationToTimeString(Number(episodeMine.file.duration)),
  //   url: episodeMine.file.url,
  //   description: episodeMine.description
  // }

  const episodeFake = {
    title: "Faladev #30 | A importância da contribuição em Open Source",
    members: "Diego Fernandes, João Pedro, Diego Haz e Bruno Lemos",
    publishedAt: format(parseISO("2021-01-22 16:35:40"), 'd MMM yy', { locale: ptBR }),
    thumbnail: "https://storage.googleapis.com/golden-wind/nextlevelweek/05-podcastr/opensource.jpg",
    durationAsString: convertDurationToTimeString(3981),
    description: "<p>Nesse episódio do Faladev, Diego Fernandes se reúne com João Pedro Schmitz, Bruno Lemos e Diego Haz, para discutir sobre a importância da contribuição open source e quais desafios circulam na comunidade.</p><p>A gente passa a maior parte do tempo escrevendo código. Agora chegou o momento de falar sobre isso. Toda semana reunimos profissionais da tecnologia para discutir sobre tudo que circula na órbita da programação.</p><p>O Faladev é um podcast original Rocketseat.</p>",
    url: "https://storage.googleapis.com/golden-wind/nextlevelweek/05-podcastr/audios/opensource.m4a",
    duration: 3981
  }
  ...
  return (
    <div className={styles.episode}>
      <div className={styles.thumbnailContainer}>
        <Head>
          <title> {episodeFake.title} | Podcastr </title>
        </Head>
        ...
        <Image
          width={700}
          height={160}
          src={episodeFake.thumbnail}
          alt={episodeFake.title}
          objectFit="cover"
        />

        <button type="button" onClick={() => play(episodeFake)}>
      ...
      <header>
        <h1> {episodeFake.title} </h1>
        <span> {episodeFake.members} </span>
        <span> {episodeFake.publishedAt} </span>
        <span> {episodeFake.durationAsString} </span>
      </header>

      <div className={styles.description} dangerouslySetInnerHTML={{ __html: episodeFake.description }} />
